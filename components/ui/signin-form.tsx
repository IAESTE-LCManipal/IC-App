// SigninForm component for dynamic import in signin page
"use client";
...
export default function SigninForm() {
  ...
  return (
    <div className='rounded-lg border-2 border-gray-700 py-2 w-[400px] max-w-full'>
      <Image src={lcmu} alt='lcmu' className='w-40 sm:w-60 h-auto mx-auto mt-4' loading="lazy" />
      {error && (
      ...
      <Tabs defaultValue="intern" className="w-full bg-gray-900 text-white p-4 rounded-lg">
          ...
          <Card className="bg-gray-800 text-white border border-gray-800">
            ...
            <CardContent className="space-y-4">
              <div>
                ...
                <Input id="intern-id" placeholder="Intern ID" className="bg-gray-700 text-white border border-gray-600" value={internID} onChange={e => setInternID(e.target.value)} />
              </div>
              <div>
                <Label htmlFor="intern-password" className="text-white text-sm sm:text-base">Password</Label>
                <Input id="intern-password" type="password" placeholder="Password" className="bg-gray-700 text-white border border-gray-600" value={internPassword} onChange={e => setInternPassword(e.target.value)} />
              </div>
            </CardContent>
            <CardFooter>
              <Button className="w-full bg-blue-600 hover:bg-blue-700" onClick={handleInternSignIn} disabled={isLoading}>
        ...
        <TabsContent value="lc">
          <Card className="bg-gray-800 text-white border border-gray-800">
            <CardHeader>
              <CardTitle className="text-lg sm:text-xl">LC/Admin</CardTitle>
              <CardDescription className="text-sm sm:text-base">LC/Admin Login page.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div>
            ...
            <CardFooter>
              <Button className="w-full bg-blue-600 hover:bg-blue-700" onClick={handleLcSignIn} disabled={isLoading}>
                {isLoading ? "Signing in..." : "Sign In"}
              </Button>
            </CardFooter>
          </Card>
